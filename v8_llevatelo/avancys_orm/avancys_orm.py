...
def fast_delete(cursor, model, where):
    """
    Llamar para eliminar rapidamente registros que tengan triggers y demore el proceso
    @params:
        cursor - req: self.env.cr o cr en api vieja
        modelo - req: Nombre de la tabla
        where - req: tupla de dos indices con el campo y el valor, el cual puede ser un item o lista de items
    """
    if type(where[1]).__name__ == 'list':
        where = [where[0], tuple(where[1]) if len(where[1]) > 1 else tuple([where[1][0], 0])]
        ...
        where = [where[0], tuple([where[1], 0])]
    odor = "ALTER TABLE {table} DISABLE TRIGGER ALL".format(table=model)
    delete = "DELETE FROM {table} WHERE {crit} in {critval}".format(table=model, crit=where[0], critval=where[1])
    cdor = "ALTER TABLE {table} ENABLE TRIGGER ALL".format(table=model)
    cursor.execute(odor)
