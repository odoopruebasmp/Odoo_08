...
class StockReportKardexWizard(models.TransientModel):
    ...
    def kardex_compute(self):
        ...
        if self.product_ids:
            ...
            product_templates = self.env['product.template'].search([('type', '=', 'product'), ('is_asset', '=', False)])
            templates_ids = tuple(p.id for p in product_templates)
            product_list = self.env['product.product'].search([('product_tmpl_id', 'in', templates_ids)])
            if not product_list:
                raise Warning('No hay productos para generar el informe')
        ...
        sql_tz_offset = '+ interval \'{hours}\' + interval \'{minutes}\''.format(
            hours=tz_offset[:-2] + ' hours',
            minutes=tz_offset[-2:] + ' minutes'
        )
        if self.print_report == 'print':
            self._cr.execute(''' SELECT 
                                ...
                                ORDER BY 
                                    product_id, date '''.format(sql_tz_offset=sql_tz_offset))
