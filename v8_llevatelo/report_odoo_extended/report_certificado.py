...
class wizard_certificado_retencion(models.TransientModel):
    ...
    def print_report(self):
        ...
        if not account_ids:
            raise Warning('El certificado seleccionado no tiene configurada'
                          ' ninguna linea de impuestos')
        ...
        for res in result:
            ...
            if self.cert_id.is_ica:
                ...
                city = "SELECT ciudad FROM account_tax WHERE id in {taxes} GROUP BY ciudad".format(taxes=tuple(c.id for c in tax_id))
    ...
    def print_report(self, send=False):
        ...
        for employee in employee_ids:
            ...
            for concepto in self.cert_id.conceptos_ids.sorted(key=lambda y: y.sequence):
                ...
                tot_concept = []
                if concepts:
                    sum_qry = ("SELECT hpc.employee_id,"
                               "abs(sum(case when hpc.category = 'deductions' then -hpc.total else hpc.total end)),"
                               "count(hpc.id), string_agg(hpc.id::varchar,',') "
