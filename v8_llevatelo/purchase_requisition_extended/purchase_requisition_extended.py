...
class stock_transfer_details(models.TransientModel):
    ...
    def get_qty_available_per_product_per_location(self, item):
        moves = getattr(self.picking_id, 'move_lines') or []
        ...
        and sq.location_id = {2}""".format(
            move_pool, item.product_id.id, item.sourceloc_id.id
        ))
        qty_available = self._cr.fetchone()
        return (
            item.product_id.id,
            qty_available[0] if qty_available and qty_available[0] else 0.0,
            item.sourceloc_id.id
        )


    def get_qty_requested_per_product_per_location(self):
        return sorted([
            (item.product_id.id,
            sum(
                i.quantity for i in self.item_ids
                if i.product_id.id == item.product_id.id
            ),
            item.sourceloc_id.id)
            for item in self.item_ids if item.sourceloc_id.usage == 'internal'\
            and item.product_id.type == 'product'
    ...
    def validate_requested_vs_available(self, requested, available):
        ...
        return requested[qty] > available


    def _get_lot_unavailable(self, item):
        moves = getattr(self.picking_id, 'move_lines') or []
        move_pool = '(' + (','.join(str(move.id) for move in moves) or '-1')\
            + ')'
        self._cr.execute("""select sum(sq.qty) from
        ...
        """.format(
            move_pool, item.lot_id.id or -1, item.product_id.id,
                item.sourceloc_id.id
        ))
        qty_available = self._cr.fetchone()
        lot_qty_available = qty_available[0] if qty_available\
            and qty_available[0] is not None else 0.0
        qty_available_quant = float_round(lot_qty_available, precision_rounding=item.product_id.uom_id.rounding)
        item_qty = float_round(item.quantity, precision_rounding=item.product_id.uom_id.rounding)
        return item_qty > qty_available_quant


    def validate_quantities_to_transfer(self):
        qty_available_per_product_per_location = sorted([
            self.get_qty_available_per_product_per_location(item)
            for item in self.item_ids if item.sourceloc_id.usage == 'internal'\
            and item.product_id.type == 'product'
        ], key=lambda k_item: k_item[0])
        qty_requested_per_product_per_location =\
            self.get_qty_requested_per_product_per_location()
        ...
        item_lot_not_available = filter(
            lambda item: self._get_lot_unavailable(item)\
                and item.sourceloc_id.usage == 'internal'\
                and item.product_id.type == 'product'
                # and item.product_id.track_all
                and item.lot_id,
            self.item_ids
        )
        if item_lot_not_available:
            ...
            locations = separator.join([
                item.sourceloc_id.display_name for item in item_lot_not_available
            ])
            raise Warning('Esta intentando mover mayores cantidades de\
                los productos: %s a las disponibles en los lotes: %s\
                en las ubicaciones : %s' % (products, lots, locations))

    def truncate_quants_for_services(self):
        service_moves = self.picking_id.move_lines.filtered(
            lambda m: m.product_id.type == 'service')
        if not service_moves:
            return
        if self.picking_id.picking_type_id.code == 'outgoing':
            location = service_moves[0].location_id.id
        elif self.picking_id.picking_type_id.code == 'incoming':
            location = service_moves[0].location_dest_id.id
        else:
            return
        service_quants = self.env['stock.quant'].search([
            ('product_id', 'in', [m.product_id.id for m in service_moves]),
            ('reservation_id', '=', False),
            ('location_id', '=', location)    
        ])
        for sq in service_quants:
            sq.sudo().qty = 0
            self._cr.execute('delete from stock_quant where id = %s' % sq.id)

    @api.one
    def do_detailed_transfer(self):
        self.validate_quantities_to_transfer()        
        ...
        if self.picking_id.origin:
            if self.picking_id.state == 'done':
                ...
                self.truncate_quants_for_services()
        return res
...
class purchase_requisition_api(models.Model):
    ...
    def _prepare_purchase_order_line(self, cr, uid, requisition, requisition_line, purchase_id, supplier, context=None):
        ...
        vals.update({
            'product_qty': qty_new,
            'account_analytic_id':
                    requisition_line.analytic_account_id.id})
        return vals
