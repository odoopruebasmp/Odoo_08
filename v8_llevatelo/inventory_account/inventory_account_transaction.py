...
class stock_inventory(osv.osv):
    ...
    def pre_transfer_accountancy_validations(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        for inv in self.browse(cr, uid, ids, context=context):
            if inv.account_transaction_in_id and inv.account_transaction_out_id:
                ...
                if not partner_id:
                    raise Warning("Para realizar el ajuste contable debe configurar un tercero en el inventario %s."  % inv.name)
                for line_move in inv.line_ids:
                    ...
                    if change > 0:#hay ganancias
                        ...
                        if not account_id:
                            raise Warning("Cuenta de Entrada a Existencias no definda en el producto %s ." % line_move.product_id.default_code)
                        cost = line_move.product_id.standard_price
                        if cost <= 0.0 and not line_move.product_id.costo_zero:
                            raise Warning("El producto %s, tiene un costo igual o menor a cero, por favor validar o consultar con el area encargada"  % line_move.product_id.default_code)
                    else:
                        account_id = line_move.product_id.property_stock_account_input or line_move.product_id.categ_id.property_stock_account_input_categ
                        if not account_id:
                            raise Warning("Cuenta de Entrada a Existencias no definda en el producto %s ." % line_move.product_id.default_code)
                ...
                raise Warning("Para validar el ajuste de inventario, debe parametrizar las cuentas de perdida y ganancia")

    def action_done(self, cr, uid, ids, context=None):
        self.pre_transfer_accountancy_validations(cr, uid, ids, context=context)
