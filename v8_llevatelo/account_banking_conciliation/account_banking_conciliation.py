...
class AccountBankStatement(models.Model):
    ...
    def conciliate(self):
        ...
        int_trans_comp = int(trans_comp[0][0])
        int_moves_comp = int(moves_comp[0][0])
        trans2_moves = False
        moves2_trans = False

        count_trans = len(trans_comp)
        count_moves = len(moves_comp)

        if (count_trans > 1 and count_moves > 1) and count_trans != count_moves:
            raise Warning("La estructura de las transacciones seleccionadas no son correctas. \
                          \n El numero de transacciones y conciliaciones deben ser iguales \
                          \n o deben ser distribuida de uno a muchos")

        if int_trans_comp != 1 and int_moves_comp == 1:
            trans2_moves = True
        elif len(moves_comp) < len(trans_comp) and len(moves_comp) == 1:
            trans2_moves = True
        
        if int_trans_comp == 1 and int_moves_comp != 1:
            moves2_trans = True
        elif len(moves_comp) > len(trans_comp) and len(trans_comp) == 1:
            moves2_trans = True
        
        if not trans2_moves and not moves2_trans:
            if trans_comp and (trans_comp != moves_comp):
                raise Warning("La estructura de las transacciones seleccionadas no son correctas. \
                               \nCuando el número de transacciones es igual a el número de movimientos \
                               \nlos valores de los datos seleccionados debe coincidir.")
            ...
            if trans2_moves == 1:
                for move in moves:
                    self._cr.execute("""
                        ...
                        (statement_line_id, account_move_line_id)
                        VALUES ({tid}, {amid})
                        """.format(tid=transaction.id, amid=move[0]))
            elif moves2_trans == 1:
                for move in moves:
                    self._cr.execute("""
                        INSERT INTO statement_lines_move_lines_rel 
