...
class account_suppier_payment_wizard(osv.osv_memory):
    ...
    def do_process(self, cr, uid, ids, context=None):
        ...
        def strip_accents(s):
            st = unicode(s)
            new_string = ''.join(c for c in unicodedata.normalize('NFD', st) if unicodedata.category(c) != 'Mn')
                    ...
                    if line.partner_id.id not in partner_ids:
                        ...
                        if active_model in ['account.voucher','purchase.advance.supplier']:
                            ...
                            if object.type_payment == 'Pago Nomina':
                                ...
                                if not other_partner_id.default_bank_id:
                                    raise Warning('El tercero %s no tiene configurada ninguna cuenta bancaria por defecto'
                                                    % (other_partner_id.name))
                                bank_account_id = other_partner_id.default_bank_id
                            for all_line in lines:
                                if all_line.partner_id.id == line.partner_id.id and all_line.id != line.id:
                                    other_amount += all_line.amount_currency
                        if not bank_account_id:
            ...
            # Banco Comercial AV Villas S.A.:
                ...
                if active_model == 'payment.order':
                    for line in lines:
                        if line.partner_id.id not in partner_ids:
                            partner_ids.append(line.partner_id.id)
                            partner_amount_currency = sum(l.amount_currency for l in lines if l.partner_id == line.partner_id)
                            seq += 1
                            text_final += "02"
                            text_final += "000023"
                            text_final += object.mode.bank_id.state == 'ahorros' and '01' or object.mode.bank_id.state == 'corriente' and '06'
                            ...
                            text_final += str(seq).rjust(9,'0')
                            text_final += str(int(partner_amount_currency)).replace('.','').rjust(16,'0')
                            text_final += str(partner_amount_currency)[-2:].replace('.','').rjust(2,'0')
                            text_final += str(0).rjust(16,'0')
                            text_final += str(0).rjust(16,'0')
                            text_final += str(0).rjust(16,'0')
...
class AccountSupplierPaymentWizard(models.TransientModel):
    ...
    def generate_file(self):
        ...
        if self._context['active_model'] == 'payment.order':
            ...
            if config.footer_line_ids:
                footer = construct_line(payment_order_id, False, config.footer_line_ids, False, ctx, True)
        ...
        if config.footer:
            final_content = body + header + footer
        else:
            final_content = header + body + footer
