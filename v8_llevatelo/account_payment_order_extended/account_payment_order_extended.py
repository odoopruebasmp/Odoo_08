        ...
        for payment in self.browse(cr, uid, ids, context=context):
            if payment.line_ids:
                ...
                if summarized_payment:
                    ...
                    ban = 0
                    # Validaciones previas al crear lineas.
                    payment_hint = (
                            '. Por favor redistribuya las cantidades de manera '
                            'que quede maximo una linea con pago parcial y '
                            'el resto de lineas con pago total.'
                        )
                    sorted_by_partner = payment.line_ids.sorted(lambda line:line.partner_id.id)
                    for dummy, paylines_iter in  groupby(sorted_by_partner, key=lambda line: line.partner_id.id):
                        paylines = list(paylines_iter)
                        partial_payments = filter(lambda payline: (
                                payline.amount_currency < payline.move_line_id.amount_residual_currency
                                <= payline.move_line_id.credit),
                                paylines)
                        if len(partial_payments) > 1 and len(paylines) > 1:
                            hint_lines = ', '.join(line.name for line in partial_payments)
                            raise ValidationError(
                                'Esta intentando pagar parcialmente '
                                'mas de una linea por tercero. ' + hint_lines + payment_hint)
                    exceeded_payments = filter(lambda payline: (
                            payline.amount_currency > payline.move_line_id.credit),
                            payment.line_ids)
                    if exceeded_payments:
                        hint_lines = ', '.join(line.name for line in exceeded_payments)
                        raise ValidationError(
                            'Esta intentando pagar mas de lo adeudado en '
                            'las lineas: ' + hint_lines + payment_hint
                            )
                    ...
                    for lines in payment.line_ids:
                        ...
                        total_reconcile = lines.amount_currency == lines.move_line_id.credit
                        ckey = (lines.bank_id.id, lines.other_partner_id.id, total_reconcile)
                        dkey = (lines.bank_id.id, move_line_data_debit['partner_id'],
                                move_line_data_debit['account_id'], total_reconcile)
                    ...
                    for concil in filter(lambda vals: not vals[3], dconcil):
                        if len(concil) >= 2:
                            account_move_line_obj.reconcile_partial(cr, uid, dconcil[concil], writeoff_acc_id=False,
                                                                    writeoff_period_id=False, writeoff_journal_id=False)
                    for concil in filter(lambda vals: vals[3], dconcil):
                        if len(concil) >= 2:
                            account_move_line_obj.reconcile(cr, uid, dconcil[concil], writeoff_acc_id=False,
                                                                    writeoff_period_id=False, writeoff_journal_id=False)
...
class account_voucher(models.Model):
    ...
    def create_analytic_lines(self):
        for move in self.move_ids.filtered(
            lambda move_line: (
                move_line.account_id.user_type.report_type.lower()
                in ['income', 'expense'])):
    ...
    def proforma_voucher(self, cr, uid, ids, context=None):
        ...
        aag = self.pool.get('ir.module.module').search(
            cr, uid, [
                ('name', '=', 'account_analytic_general'),
                ('state', '=', 'installed')
            ], context=context)
        for voucher in self.browse(cr, uid, ids, context=context):
            if voucher.analytic_account_id and not aag:
                if not voucher.journal_id.analytic_journal_id.id:
                    raise Warning('No se ha encontrado un comprobante analítico'
                        ' para el diario: ' + voucher.journal_id.name)
                voucher.create_analytic_lines()
        return True
...
class FilePaymentConfig(models.Model):
    ...
    footer_line_ids = fields2.One2many('acocunt.payment.config.line', 'footer_config_id', 'Lineas')
...
class FilePaymentConfigLine(models.Model):
    ...
    footer_config_id = fields2.Many2one('account.payment.file.config', 'Configuracion Pie')
