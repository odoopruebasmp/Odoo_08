...
class StockPicking(models.Model):
    ...
    def compute_invoice(self, cr, uid, ids, invoice_id, context=None):
        ...
        if not invoice.stock_picking_id:
            raise Warning('No es posible realizar la operacion:\
                No se encuentra una transferencia asociada a esta factura,\
                Verifique que exista el campo Stock Move')
        picking_id = invoice.stock_picking_id.id
        res = super(StockPicking, self).compute_invoice(cr, uid, [picking_id], invoice_id, context=context)
