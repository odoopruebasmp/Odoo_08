...
class hrDismissalMassive(models.Model):
    ...
    payslip_period_id = fields.Many2one('payslip.period', string='Periodo Liquidacion', required=True,
                                        help='Este campo permite indicar el periodo en el cual se realizará la liquidación de los contratos seleccionados.', domain=[('closed', '=', False)])
    ...
    suffix_payslip_run_name = fields.Char(string="Liquidación de:", required=True)
    ...
    def dismissal_massive(self):
        ...
        if active_ids:
            ...
            results = {
                ...
                'name': 'Liquidacion de %s - %s' % (self.payslip_period_id.name, self.suffix_payslip_run_name),
            ...
            qty_contracts = len(contract_ids)
            for i,contract in enumerate(contract_ids):
                _logger.info('Liquidacion %s de %s'%(i+1,qty_contracts))
...
class hrContractExtended(models.Model):
    ...
    @api.multi
    def create_payslip(self, payslip_run_id=False):
        ...
        for contract in self:
            ...
            notes += "Dia final del contrato: %s\n"
            notes += "Total(Ultimos 12 meses): %s\n"
            notes += "Salario Mensual: %s\n"
            notes += "Salario diario: %s\n"
            notes += "Ultimo Salario: %s\n"
            notes = notes % (
            ...
            if payslip_run_id and type(payslip_run_id) == int:
                results['payslip_run_id'] = payslip_run_id
            _logger.info('Generando Liquidacion del contracto id = %s'%(contract.id))
