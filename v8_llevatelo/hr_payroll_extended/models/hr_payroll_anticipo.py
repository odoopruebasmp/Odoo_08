...
class hr_payroll_advance(osv.osv, EDIMixin):
    ...
    def onchange_aprove_date(self, cr, uid, ids, currency_id, company_id, approve_date, context=None):
        val = {}
        warning = {}
        ...
        if currency_id:
            ...
            if company.currency_id.id != currency_id:
                ...
                cr.execute("select id from res_currency_rate where currency_id = {c} and "
                           "name::varchar like '{d}%'".format(c=currency_id, d=approve_date))
                ...
                if not currency_date_id:
                    warning = {
                        'title': _('Advertencia!'),
                        'message' : _("No existe la tasa de cambio para la fecha '%s' ") % (approve_date,)
                    }
                    rate = -1
                else:
        ...
        for advance in self.browse(cr, uid, ids, context):
            ...
            if vals.get('currency_id',False) and not vals.get('tasa_cambio',False):
                ...
                if company.currency_id.id != currency_id:
                    ...
                    if advance.approve_date:
                        date = advance.approve_date
                        
                    currency_obj = self.pool.get('res.currency.rate')
                    currency_date_id = currency_obj.search(cr, uid, [('currency_id','=',currency_id)])
                    for currency_date in currency_date_id:
                        currency_date = currency_obj.browse(cr, uid, currency_date, context=context)[0].name
                        if currency_date[:10] == date:
                            currency_date_id = currency_date_id
                            break
                    if not currency_date_id:
                        rate = -1
                    else:
            ...
            elif vals.get('approve_date', False):
                currency_id = advance.currency_id
                date = vals.get('approve_date', False)
                currency_obj = self.pool.get('res.currency.rate')
                currency_date_id = currency_obj.search(cr, uid, [('currency_id','=',currency_id.id)])
                for rate_id in currency_date_id:
                    currency_date = currency_obj.browse(cr, uid, rate_id, context=context)[0].name
                    if currency_date[:10] == date:
                        currency_date_id = rate_id
                        break
                if not currency_date_id:
                    rate = -1
                else:
