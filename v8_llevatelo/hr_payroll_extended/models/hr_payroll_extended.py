...
def unaccent(data):
    if type(data) == str:
        return unicode(data, "latin1").encode("ascii","replace").replace("?","")
    elif type(data) == unicode:
        return data.encode("ascii","replace").replace("?","")
    else:
        return data
...
class hr_employee(osv.osv, EDIMixin):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        ctx = context.copy()
        if partner_id and not context.get('do_not_repeat'):
            ctx.update({'do_not_repeat':True})
            partner_obj = self.pool.get('res.partner')
            partner = partner_obj.browse(cr, uid, partner_id,context=ctx)
            vals.update({'user_id':partner.system_user_id.id})

            for empleado in self.browse(cr, uid, ids, context=ctx):
                partner_ids = partner_obj.search(cr, uid, [('employee_id','=',empleado.id)], context=ctx)
                if partner_ids:
                    partner_obj.write(cr, uid, partner_ids, {'employee_id': False}, context=ctx)

            partner_obj.write(cr, uid, [partner_id], {'employee_id': ids[0]}, context=ctx)
        return super(hr_employee, self).write(cr, uid, ids, vals, context=ctx)
    ...
    _columns = {
        ...
        'marital': fields.selection([('single', 'Single'), ('married', 'Married'), ('widower', 'Widower'),
                                     ('divorced', 'Divorced'), ('common-law relationship','Union Libre')], 'Marital Status'),
    }
...
class hr_contract(osv.osv):
    ...
    _columns = {
        ...
        'fiscal_type_id': fields.many2one('hr.fiscal.type', "Tipo de Cotizante", track_visibility='onchange'),
        ...
        'payslip_period_id': fields.many2one('payslip.period', "Periodo Liquidacion", domain=[('closed', '=', False)]),
        'separation_type': fields.selection(RETIROS, 'Tipo de Finalizacion', track_visibility='onchange'),
        ...
        'prod_ind': fields.boolean('Afecta costo indirecto de producción'),
    ...
    def close_contract_nolab (self, cr, uid, ids, context=None):
        hr_contract_obj = self.pool.get('hr.contract')
        for contract in hr_contract_obj.browse(cr,uid, ids, context=context):
            if not contract.date_end:
                raise Warning ('Asigne una Fecha liquidacion al contrato = {C}'.format(C=contract.name))
            if not contract.payslip_period_id:
                raise Warning ('Asigne un Periodo Liquidacion al contrato = {C}'.format(C=contract.name))
            contract.write({'state':'done'})

    def open_contract_nolab (self, cr, uid, ids, context=None):
        hr_contract_obj = self.pool.get('hr.contract')
        for contract in hr_contract_obj.browse(cr,uid, ids, context=context):
            contract.write({'state':'in_progress', 'date_end':None,'payslip_period_id':None, 'separation_type':None})

class HrContract(models.Model):
    _name = 'hr.contract'
    _inherit = ['hr.contract','mail.thread']

    date_start = fields2.Date('Start Date', required=True, track_visibility='onchange')
    date_end = fields2.Date('End Date', track_visibility='onchange')
    type_id = fields2.Many2one('hr.contract.type', "Contract Type", required=True, track_visibility='onchange')
    analytic_account_id = fields2.Many2one('account.analytic.account', 'Analytic Account', track_visibility='onchange')
    workcenter = fields2.Char('Centro de Trabajo')
    ...
    def create_payslip_reliquidation(self):
        ...
        """
        payslip_type_liq = self.env['hr.payslip.type'].search([('code','=','Liquidacion')])
        new_payslip_ids = []
        contrac_distribution_analytic = """
            select IMF.id from ir_model_fields as IMF   inner join ir_model as IM
            on IM.id = IMF.model_id where IMF.name = 'analytic_account_ids' and IM.model='hr.contract'"""
        contrac_distribution_analytic = orm.fetchall(self._cr, contrac_distribution_analytic)

        if not payslip_type_liq:
            raise Warning('Debe configurar en los tipos de nomina, un tipo con el codigo <Liquidacion>')
        elif len(payslip_type_liq) > 1:
            raise Warning('Se encontraron {N} tipos de nomina con el codigo <Liquidacion>'.format(N=len(payslip_type_liq)))
        for contract in self:
            ...
            if not payslips_ids:
                raise Warning('Error', 'Debe crear primero una nomina de tipo <Liquidacion> para el contrato {C}'.format(C=contract.name))
            elif no_done_payslips:
                raise Warning ('Error', 'Este proceso se debe hacer unicamente para ajustar la nomina de tipo <Liquidacion> que ya esta causada',
                'Se encontraron las nominas {N} de tipo <Liquidacion> en estado {E}'.format(N=[p.number for p in no_done_payslips],E=[p.state for p in no_done_payslips]))
            analytic_eval = [aaa.analytic_account_id.name for aaa in contract.analytic_account_ids] if contrac_distribution_analytic else [contract.analytic_account_id.name]
            not_analytic_eval = False
            for ae in analytic_eval:
                if ae:
                    not_analytic_eval = True
                    break
            if not contract.employee_id or not contract.payslip_period_id or not contract.journal_id or not not_analytic_eval:
                message = 'Del contrato {C} la siguite informacion es errornea\n'.format(C=contract.name)
                message += '    -Empleado = {E}\n'.format(E=unaccent(contract.employee_id.name) if contract.employee_id else False)
                message += '    -Periodo liquidacion = {P}\n'.format(P=unaccent(contract.payslip_period_id.name) if contract.payslip_period_id else False)
                message += '    -Libro de Salarios = {L}\n'.format(L=unaccent(contract.journal_id.name) if contract.journal_id else False)
                message += '    -Cuenta(s) Analíticas = {C}\n'.format(C=[unaccent(aaa) for aaa in analytic_eval] if analytic_eval else False)
                raise Warning('Error', message)
            notes = 'Reliquidacion de {E}\n'.format(E=unaccent(contract.employee_id.name))
...
class hr_payslip(osv.osv, EDIMixin):
    ...
    def _compute_extra_hours(self, cr, uid, ids, name, args, context=None):
        ...
        for payslip in self.browse(cr, uid, ids, context=context):
            if payslip.tipo_nomina.code in ('Nomina', 'Liquidacion', 'Otros'):
                res[payslip.id] = extrahours_obj.search(cr, uid,
                                                        ['|', ('state', '=', 'validated'),
    ...
    def _compute_novedades(self, cr, uid, ids, name, args, context=None):
        ...
        for payslip in self.browse(cr, uid, ids, context=context):
            if payslip.tipo_nomina.code in ('Nomina', 'Otros', 'Cesantias Parciales', 'Vacaciones', 'Prima'):
    ...
    def _compute_prestamos(self, cr, uid, ids, name, args, context=None):
        ...
        for payslip in self.browse(cr, uid, ids, context=context):
            if payslip.tipo_nomina.code in ('Nomina', 'Vacaciones', 'Liquidacion'):
                res[payslip.id] = prestamos_obj.search(cr, uid,
                                                       ['|',('payslip_id', '=', False),('payslip_id', '=', payslip.id),
                                                        ('date', '>=', payslip.payslip_period_id.start_date),
                                                        ('date', '<=', payslip.payslip_period_id.end_date),
                                                        ('contract_id', '=', payslip.contract_id.id),
                                                        ('state', '=', 'validated')], order='date asc')
            else:
                res[payslip.id] = []
        return res
    ...
    def _compute_advances(self, cr, uid, ids, name, args, context=None):
        ...
        for payslip in self.browse(cr, uid, ids, context=context):
            if payslip.tipo_nomina.code in ('Nomina','Liquidacion'):
                advances = []
    ...
    _columns = {
        ...
        'payslip_period_id': fields.many2one('payslip.period', "Period", domain=[('closed', '=', False)], states={'draft': [('readonly', False)]}),
    ...
    def compute_obligaciones(self, cr, uid, ids, context=None):
        ...
        for payslip in self.browse(cr, uid, ids, context=context):
            ...
            las_obligaciones = {}
            obli_aplican = obligaciones_obj.search(cr, uid,
                                                   [('state', '=', 'validated'),
                                                    ('contract_id', '=', payslip.contract_id.id),
                                                    ('date_from', '<=', payslip.payslip_period_id.start_date),
                                                    ('date_to', '>=', payslip.payslip_period_id.end_date)],
    ...
    def onchange_employee_id(self, cr, uid, ids, employee_id, contract_id, context=None):
        ...
        if employee_id:
            ...
            res['domain'] = {'contract_id': [('id', 'in', contract_ids)], 
                             'payslip_period_id': [('id', 'in',period_ids), ('closed', '=', False)]}
        return res
    ...
    def get_worked_day_lines_new(self, cr, uid, slip, context=None):
        ...
        if hp_type in ps_types:
            ...
            dtl = []
            for day_per in daterange(dt_sp, dt_ep):
                str_dt = datetime.strftime(day_per, "%Y-%m-%d")
                dr[str_dt] = ('WORKED', 'unpaid')
                dtl.append(str_dt)
            dtt = tuple(dtl)

            # Reduccion por inicio o fin de contrato
            if dt_sp < dt_ksd:
                for day_per in dr:
                    if day_per < k_start_date:
                        dr[day_per] = ('START_DED', 'unpaid')

            if dt_ked and dt_ked <= dt_ep:
                for day_per in dr:
                    if day_per > k_end_date:
                        if day_per[8:10] != '31':
                            dr[day_per] = ('END_DED', 'unpaid')
                        else:
                            dr[day_per] = ('IGNORE', 'unpaid')


            ld_query = ("SELECT hhd.id, hhd.name, hhd.days_payslip, hhs.code, "
                        "hhd.holiday_status_id, hhd.holiday_id, hhd.state, hhs.name "
                        "FROM hr_holidays_days hhd "
                        "LEFT JOIN hr_holidays_status hhs on hhs.id = hhd.holiday_status_id "
                        "WHERE hhd.name IN {days} "
                        "AND hhd.contract_id = {k} "
                        "AND hhd.state IN ('validate', 'paid') "
                        "ORDER BY hhd.name ASC".format(days=dtt, k=contract.id)
                        )

            # Aplicacion de ausencias
            ld_fetch = orm.fetchall(cr, ld_query)
            ld_dict = {}
            leave_types = []
            for ld in ld_fetch:
                ld_dict[ld[1]] = ld
                if ld[1] in dr:
                    dr[ld[1]] = (ld[3], 'paid' if ld[6] == 'paid' else 'unpaid')
                    if ld[3] not in leave_types:
                        leave_types.append([ld[3], ld[7]])

            leav_query = ("SELECT ")

            wd_q = ("SELECT hpd.datestr, hpd.code, hpd.state "
                    "FROM hr_payslip_day hpd "
                    "INNER JOIN hr_payslip hp ON hp.id = hpd.payslip_id "
                    "WHERE hpd.datestr IN {days} "
                    "AND hp.contract_id = {cid} "
                    "AND hpd.code = 'WORKED' ".format(cid=contract.id, days=dtt))
            wd_data = orm.fetchall(cr, wd_q)
            for wdt in wd_data:
                # Dias trabajados pagados en otras nominas
                if wdt[0] in dr and dr[wdt[0]][0] not in ('START_DED', 'END_DED'):
                    dr[wdt[0]] = ('WORKED', 'paid')
                elif wdt[0] in dr and dr[wdt[0]][0] in ('START_DED', 'END_DED'):
                    dr[wdt[0]] = ('REFUND', 'unpaid')

            # Reduccion por nomina de vacaciones
            if hp_type == 'Vacaciones' and not payslip.company_id.fragment_vac:
                max_per_date = max(ld_fetch, key=lambda i: i[1] if i[3] == 'VAC' else 0)[1]
                for day_per in dr:
                    if day_per > max_per_date:
                        dr[day_per] = ('RANGE_OUT', 'unpaid')

            worked = sum(value[0] == 'WORKED' and value[1] == 'unpaid' for value in dr.values())
            work_payed = sum(value[0] == 'WORKED' and value[1] == 'paid' for value in dr.values())

            if work_payed:
                wd_payed = {
                    'name': _("Días Trabajados pagados previamente"),
                    'sequence': 6,
                    'code': 'WD_PAYED',
                    'symbol': '',
                    'number_of_days': work_payed,
                    'number_of_hours': work_payed * w_hours,
                    'contract_id': contract.id
                }
                res += [wd_payed]

            refund = sum(value[0] == 'REFUND' and value[1] == 'unpaid' for value in dr.values())
            worked -= refund

            start_ded = sum(value[0] == 'START_DED' for value in dr.values())
            if start_ded:
                start_ded = {
                    ...
                    'code': 'START_DED',
                    'symbol': '-',
                    'number_of_days': start_ded,
                    'number_of_hours': start_ded * w_hours,
                    'contract_id': contract.id
                }
                res += [start_ded]

            end_ded = sum(value[0] == 'END_DED' for value in dr.values())
            if end_ded:
                end_ded = {
                    ...
                    'code': 'END_DED',
                    'symbol': '-',
                    'number_of_days': end_ded,
                    'number_of_hours': end_ded * w_hours,
                    'contract_id': contract.id
                }
                res += [end_ded]
            ...
            ro_total = sum(value[0] == 'RANGE_OUT' for value in dr.values())
            if ro_total:
                ro_days = {
                    'name': _("Días fuera de rango"),
                    'sequence': 8,
                    'code': 'RANGE_OUT',
                    'symbol': '',
                    'number_of_days': ro_total,
                    'number_of_hours': ro_total * w_hours,
                    'contract_id': contract.id
                }
                res.append(ro_days)

            for lt in leave_types:
                lt_total = sum(value[0] == lt[0] for value in dr.values())
                leaves_worked_lines[lt[0]] = {
                    'name': lt[1],
                    'sequence': 4,
                    'code': (lt[0] or 'nocode'),
                    'symbol': '-',
                    'number_of_days': lt_total,
                    'number_of_hours': lt_total * w_hours,
                    'contract_id': contract.id}
        ...
        orm.direct_create(cr, uid, 'hr_payslip_day', pd_data)
        ...
        if hp_type in ps_types:
            ...
            if dt_ked and dt_ked <= dt_ep:
                ...
                if dt_ep.day == 31 and ded_end_days:
                    ded_end_days -= 1
                if dt_ked.month == 2 and (period.schedule_pay == 'monthly' or (period.schedule_pay == 'bi-monthly' and period.bm_type == 'second')):
            ...
            for leave in payslip.leave_days_ids:
                ...
                qty_hours = leave.hours_payslip
                if leave.holiday_status_id.vacaciones and leave.state == 'paid':
                    prev_pays['number_of_days'] += qty_days
                    prev_pays['number_of_hours'] += qty_days * w_hours
                    worked -= qty_days
                    continue
                pay_day_31 = True if leave.holiday_status_id.apply_payslip_pay_31 else False
                if (not pay_day_31 and leave.name[-2:] == '31') or leave.state != "validate":
                    continue
                elif pay_day_31 and leave.name[-2:] == '31':
                    worked += 1
                if leave.holiday_status_id.sub_wd:
                    worked -= qty_days
            ...
            if period.bm_type:
                if period.bm_type == 'first':
                    bmt_wh = "AND (pp.bm_type = '{bm_type}' OR pp.bm_type is Null) ".format(bm_type=period.bm_type)
                else:
                    bmt_wh = "AND (pp.bm_type = '{bm_type}' OR " \
                             "(pp.bm_type is Null and hp.liquidacion_date between '{sp}' and '{ep}')) ".format(
                                    bm_type=period.bm_type, sp=start_period, ep=end_period)
            else:
                bmt_wh = " "
            ...
            if sch_pay == 'monthly':
                ...
                if period.bm_type == 'first':
                    sch_wh = "AND (pp.schedule_pay = '{sch_pay}' OR pp.schedule_pay = 'monthly') ".format(
                        sch_pay=sch_pay)
                else:
                    sch_wh = "AND (pp.schedule_pay = '{sch_pay}' OR " \
                             "(pp.schedule_pay = 'monthly' and hp.liquidacion_date between '{sp}' and '{ep}')) ".format(
                                  sch_pay=sch_pay, sp=start_period, ep=end_period)
            ...
            ot_wd = ("SELECT wd.number_of_days, hp.id, pp.bm_type, wd.symbol, wd.code "
                     ...
                     "AND hpt.code in ('Vacaciones', 'Nomina', 'Liquidacion')".format(
            ...
            if None in types and 'first' in types and 'second' in types:
                ...
                if sch_pay == 'monthly':
                    wd_plus = sum([x[0] if x[3] in ('+', '') and x[4] != 'WORK102' else 0 for x in wd_other_data])
                else:
                    wd_plus = 0
                wd_prev = sum([x[0] if x[4] in ('PREV_AUS', 'PREV_PAYS') else 0 for x in wd_other_data])
                wd_minus = sum([x[0] if x[3] in ('-','') and x[4] not in ('DED_INICIO', 'DED_FIN', 'VAC','VAC_DISF') else 0 for x in wd_other_data])

                sum_wdo = wd_plus + wd_minus - wd_prev
                if sch_pay == 'monthly':
                    wd_other = sum_wdo
                else:
                    wd_other = sum_wdo if sum_wdo <= 15 else 15
            # #############################################
            ...
            #-----------------------------------------------------------------------------------
            #----------------------- Arreglo dias no trabajados en nomina tipo vacaciones
            #-----------------------------------------------------------------------------------
            """ if vac:
                dias_vacaciones = payslip.leave_days_ids
                periodo_nomina = payslip.payslip_period_id
                date_end_period = datetime.strptime(periodo_nomina.end_period,'%Y-%m-%d')
                end_vac = max([v.name for v in dias_vacaciones if v.state == 'validate'])
                date_end_vac = datetime.strptime(end_vac, '%Y-%m-%d')

                if date_end_period > date_end_vac and date_end_period.month == date_end_vac.month:
                    delta_days = (date_end_period - date_end_vac).day
                    worked -= delta_days """
...
class hr_payroll_run(osv.osv, EDIMixin):
    ...
    _columns = {
        'payslip_period':fields.many2one("payslip.period", "Periodo", domain=[('closed', '=', False)] ),
    ...
    def check_repetidos(self, cr, uid, ids, context=None):
        for estructura in self.browse(cr, uid, ids, context=context):
            for cuenta_regla in estructura.rules_account_ids:
                repetidos_ids = self.pool.get('hr.payroll.structure.accounts').search(cr, uid,
                                                  [('structure_id', '=', cuenta_regla.structure_id.id),
                                                   ('salary_rule_id', '=', cuenta_regla.salary_rule_id.id),
                                                   ('id', '<>', cuenta_regla.id)],context=context)
