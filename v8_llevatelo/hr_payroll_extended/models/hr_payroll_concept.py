...
class HrConcept(models.Model):
    ...
    prod_ind_debit = fields.Many2one('account.account', 'Debito Costo Indirecto Produccion')
    prod_ind_credit = fields.Many2one('account.account', 'Credito Costo Indirecto Produccion')
    documentation = fields.Html('Documentacion')
    is_retention = fields.Boolean('Retencion')
    code_tax = fields.Many2one('account.tax.code','Codigo de impuesto')
...
class HrPayslipConcept(models.Model):
    ...
    def get_accounts(self):
        ...
        if self.payslip_id.contract_id.prod_ind:
            debit_field = 'prod_ind_debit'
            credit_field = 'prod_ind_credit'
        partner_w = ""
        partner_other = False
...
class ResCompany(models.Model):
    ...
    positive_net = fields.Boolean('Cierre de nomina solo con neto positivo')
...
class HrPayslip(models.Model):
    ...
    def update_vac_book(self, payslips):
        if not payslips:
            return True
        start_vac = self.env.user.company_id.init_vac_date
        contract_ids = """
        select contract_id from hr_payslip as hp where id in {ids}
        """.format(ids=payslips)
        contract_ids = [x[0] for x in orm.fetchall(self._cr, contract_ids)]
        if not contract_ids:
            return True
        contract_ids = tuple(contract_ids if len(contract_ids)>1 else [contract_ids[0],0])
        vac_book_old = """
        select id from hr_vacation_book where contract_id in {ids} and payslip_id is not Null
        """.format(ids=contract_ids)
        vac_book_old = [x[0] for x in orm.fetchall(self._cr, vac_book_old)]
        if vac_book_old:
            orm.fast_delete(self._cr, 'hr_vacation_book', ('id', vac_book_old))

        vac_sel = """
        select hh.payslip_id, hh.number_of_days,hh.payed_vac, hp.contract_id
        from hr_payslip as hp
        inner join hr_contract as hc
            on hc.id = hp.contract_id
        inner join hr_contract_type as hct
            on hct.id = hc.type_id
        inner join hr_holidays hh
            on hh.contract_id = hc.id
        inner join hr_holidays_status hhs
            on hhs.id = hh.holiday_status_id
        where hp.id in {ids}
            and hct.type_class != 'apr'
            and hh.at_date > '{init_vac}'
            and hhs.vacaciones = 't'
            and hh.state = 'paid' """.format(ids=payslips, init_vac=start_vac)
        vacs = orm.fetchall(self._cr, vac_sel)
        upd_book = []
        for v in vacs:
            payslip = v[0] if v[0] is not None else False
            if not payslip or not v[3]:
                continue
            upd_book.append({
        ...
        #Tiene encuenta los dias pagados en las liquidaciones
        qty_days_vac_liq = """
        select HPC.qty, HP.id, HP.contract_id
        from hr_payslip_concept as HPC
        inner join hr_payslip as HP
            on HP.id = HPC.payslip_id
        inner join payslip_period as PP
            on PP.id = HP.payslip_period_id
        where HPC.code = 'VAC_LIQ' and HP.state = 'done'
            and HP.contract_id in {ids}
            and PP.start_period >= '{init_vac}'
        limit 1""".format(ids=contract_ids, init_vac=start_vac)
        for vac_liq_slip in orm.fetchall(self._cr, qty_days_vac_liq):
            upd_book.append({
                'enjoyed': vac_liq_slip[0],
                'payslip_id': vac_liq_slip[1],
                'contract_id': vac_liq_slip[2],
            })
        end_period = self.payslip_period_id.end_period if len(self) == 1 else self[0].payslip_period_id.end_period
        sus_sel = """
        select sum(HHD.days_payslip), HHD.payslip_id, HHD.contract_id
        from hr_holidays_days as HHD
        inner join hr_holidays_status HHS
            on HHS.id = HHD.holiday_status_id and HHS.no_payable
        where HHD.name >= '{init_vac}' and HHD.name <= '{end_period}'
            and HHD.state = 'paid' and HHD.contract_id in {ids}
        group by HHD.payslip_id, HHD.contract_id
        """.format(init_vac=start_vac, end_period=end_period, ids=contract_ids)
        for sus_paid in orm.fetchall(self._cr, sus_sel):
            upd_book.append({
                'licences': sus_paid[0] or 0,
                'payslip_id': sus_paid[1],
                'contract_id': sus_paid[2]
            })
        orm.direct_create(self._cr, self._uid, 'hr_vacation_book', upd_book)
    ...
    def compute_slip(self):
        ...
        self.update_vac_book(self_ids)
        for slip in slips2comp:
            # Validacion de duplicidad en trikey emp-tipo-periodo
            constraint = ("SELECT id FROM hr_payslip "
                          "WHERE contract_id = {e} AND payslip_period_id = {p} "
                          "AND tipo_nomina = {t} AND id != {i}".format(e=slip.contract_id.id,
    ...
    def compute_concepts(self):
        ...
        last_wage_period = """
        select wage from hr_contract_salary_change
        where contract_id = {C} and date <= '{date}'
        order by date desc limit 1""".format(C=contract.id, date=period.end_period)
        last_wage_period = orm.fetchall(self._cr, last_wage_period)
        wage = last_wage_period and last_wage_period[0][0] or contract.wage if not contract.smmlv else sal_min_ev
        ...
        orm.fast_delete(self._cr, 'hr_payslip_concept', ('payslip_id', self.id))
        # self._cr.execute("UPDATE hr_payslip_concept set total=0, payslip_id = Null where payslip_id = %s" % self.id)
        ...
        if self.leave_ids:
            ...
            aus_otros_periodos = self.leave_ids.filtered(lambda x: not x.holiday_status_id.general_illness
                                                 and not x.holiday_status_id.maternal_lic
                                                 and not x.holiday_status_id.paternal_lic
                                                 and not x.holiday_status_id.atep
                                                 and not x.holiday_status_id.vacaciones)
            for leave in aus_otros_periodos:
                aust = leave.holiday_status_id
                if leave.holiday_status_id.sub_wd:
                    for leave_day in leave.line_ids:
                        prev_period = leave_day.name[:7] < period.start_period[:7]
    ...
    def prep_concept(self, concept, category, cname, data, name="", origin='regular',
        ...
        if total == '':
            total = round(data[0] * data[1] * data[2] / 100, precision)
        else:
    ...
    def ibcma(self, date, contract=False):
        ...
        orig_exc = ('VAC_PAG', 'VAC_LIQ', 'PRIMA', 'PRIM_LIQ')
    ...
    def get_prst(self, date_start, date_end, ld, include=False, prst='False', nocesly=False):
        ...
        if self.env.user.company_id.prst_wo_susp and prst=='prima':
            susp = 0
        else:
        ...
        if not wage_change:
            ...
            for sgmt in wag_seg:
                if s == segments:
                    ...
                    susp_to_appy = 0
                itv = days360(sgmt[0], sgmt[1], method_eu=True) - susp_to_appy - mat_dummy
        ...
        elif self.env['ir.module.module'].search([('name','=','hr_aux_connectivity'),('state','=','installed')]):
            a_aux = ld['concepts']['AUX_CONNE']['total'] if 'AUX_CONNE' in ld['concepts'] else 0 if include else 0
            days_aux_a = ld['concepts']['AUX_CONNE']['qty'] if 'AUX_CONNE' in ld['concepts'] else 0 if include else 0
            days_aux_o = self.get_interval_concept_qty('AUX_CONNE', date_start, date_end)
            days_aux = sum([x[2] for x in days_aux_o]) + days_aux_a
            aux_itv = self.get_interval_concept('AUX_CONNE', date_start, date_end)
        ...
        if self.env['ir.module.module'].search([('name','=','hr_retroactivos'),('state','=','installed')]):
            total_variable = self.fix_total_variable_retroactivo(total_variable, date_start, date_end)
        ...
        if prst == 'ces':
            ...
            if self.tipo_nomina.code == 'Liquidacion':
                part_date_end = self.payslip_period_id.end_date
            else:
                part_date_end = date_end
            part = self.get_interval_concept('PRIMA', date_start, part_date_end)
    ...
    @api.multi
    def cumpute_table_rtf(self, base_uvt):
        """
        Tabla de retencion en la fuente 2020
        """
        rate = 0
        adj = 0
        step = 0
        ...
        if not ld['contract'].part_time:
            ...
            if ld['contract_class'] == 'reg':
                ...
                if ld['contract'].fiscal_type_id.code == '12':
                    ...
                    previos_year = datetime.strftime(datetime.strptime(ld['period'].end_period,"%Y-%m-%d") - relativedelta(years=1), "%Y-%m-%d")
                    if [x for x in self.env.ref('hr_payroll_extended.unemployment_rate_id').valores_ids if x.valor >= 10 and x.date_from < previos_year < x.date_to]:
                            lr[2] = 75.0
                    if start_period < ld['contract'].apr_prod_date <= end_period:
                        ...
                        if diff:
                            ...
                            prod_amount = ld['wage'] * (lr[2]/100) * diff / 30
    ...
    def _aux_trans(self, ld):
        ...
        sch_pay = ld['sch_pay']
        bmt = ld['bmt']
        wd = ld['wd'] - (ld['p_days']['PREV_AUS'] if 'PREV_AUS' in ld['p_days'] else 0)

        apr_prod = ld['contract'].fiscal_type_id.code == '19'
        if wd != 0 and not skip_aux:
    ...
    def _eg_b2(self, ld):
        ...
        for lday in eg_b2_days:
            if lday.name[8:10] == '31' and not lday.holiday_status_id.apply_payslip_pay_31:
                continue
    ...
    def _mat_lic(self, ld):
        ...
        if self.env.user.company_id.aus_prev:
            for leave in self.leave_ids.filtered(lambda x: x.holiday_status_id.maternal_lic):
                appv_per = ld['period'].start_period <= leave.approve_date <= ld['period'].end_period
                for leave_day in leave.line_ids:
                    if leave_day.holiday_status_id.ibc:
                        base = leave_day.holiday_id._ibc()[0] * 30
                    else:
                        base = avr
    ...
    @api.multi
    def _ices(self, ld):
        if ld.has_key('concepts'):
            if ld['concepts'].has_key('CES'):
                ces = ld['concepts']['CES']
                dias = ces['computation_obj'][2][1] + ces['computation_obj'][3][1]
                res = [float(ces['amount'])/360, dias, 12,'',ces['origin'],'',ces['computation_obj'], ces['computation']]
            else:
                res = self._ces(ld)
                res[0] = float(res[0])/360
                res[1] = res[6][2][1] + res[6][3][1]
                res[2] = 12
        else:
            raise Warning("El objeto ld no tiene la llave <concepts>")
        return res
    ...
    def _ices_part(self, ld):
        ...
        ices_part = self.get_interval_concept('ICES_PART', ref_date, ref_to_date)
        if ices_part:
            date_max = max([ices[0] for ices in ices_part])
            date_max = datetime.strptime(date_max,"%Y-%m") + relativedelta(months=1)
            date_max = datetime.strftime(date_max,"%Y-%m") + '-01'
    ...
    def _vac_liq(self, ld):
        ...
        ref_date = datetime.strptime(ld['contract'].date_end or ld['period'].end_period, "%Y-%m-%d")
        ...
        qty_sus = sum([aus.days_assigned for aus in sus_lic ])
        ...
        if ld['payslips_month']:
            ...
            select total, qty 
            from hr_payslip_concept 
            where payslip_id in {P} and code = 'VAC_LIQ' """.format(P=payslips_month)
            prev_vac_liq = orm.fetchall(self._cr, prev_vac_liq)
        ...
        if prev_vac_liq:
            ...
            lr[1] = qty
            lr[0] = (lr[0]*lr[1] - prev_vac_liq)/lr[1]
            lr[6].append(('DIAS VACACIONES LIQUIDADOS', qty))
            lr[6].append(('VACACIONES LIQUIDADAS EN OTRAS LIQUIDACIONES',prev_vac_liq))
        elif qty:
            payslip_vac = self.leave_days_ids.filtered(lambda x: x.state == 'validate' and x.holiday_status_id.vacaciones)
            qty_disf = sum([aus.days_assigned for aus in payslip_vac])
    ...
    def _ces_liq(self, ld):
        ...
        inner join payslip_period as PP
            on PP.id = HP.payslip_period_id
        where HPC.code = 'CES_LIQ'
            and HP.state = 'done'
            and HPC.contract_id = {contrato} and HPC.payslip_id != {nomina}
            and PP.start_period <= '{ep}' and PP.end_period >= '{sp}'
        """.format(contrato=ld['contract'].id, nomina=self.id, ep=ref_to_date, sp=ref_date)
        prev_ces_liq = orm.fetchall(self._cr, prev_ces_liq)
        if prev_ces_liq:
            prev_ces_liq = prev_ces_liq[0][0] if prev_ces_liq[0][0] else 0
            lr[0] = lr[0] - prev_ces_liq
            lr[6].append(('CESANTIAS LIQUIDADAS EN OTRAS LIQUIDACIONES',prev_ces_liq))
        return lr
    ...
    def _ices_liq(self, ld):
        ...
        ices = (ces_data['pres'] - ces_data['partials'])* 0.12 / 360 * days
        ...
        if net_ices > 0:
            ...
            inner join payslip_period as PP
                on PP.id = HP.payslip_period_id
            where HPC.code = 'ICES_LIQ'
                and HP.state = 'done'
                and HPC.contract_id = {contrato} and HPC.payslip_id != {nomina}
                and PP.start_period <= '{ep}' and PP.end_period >= '{sp}'
            """.format(contrato=ld['contract'].id, nomina=self.id, ep=ref_to_date, sp=ref_date)
            prev_ices_liq = orm.fetchall(self._cr, prev_ices_liq)            
            if prev_ices_liq:
                prev_ices_liq = prev_ices_liq[0][0] if prev_ices_liq[0][0] else 0
                lr[0] = lr[5] - prev_ices_liq
                lr[5] = lr[0]
                lr[6].append(('INTERESES DE CESANTIAS LIQUIDADAS EN OTRAS LIQUIDACIONES',prev_ices_liq))
        else:
            lr[3] = 'na'
        return lr
    ...
    def _prim_liq(self, ld):
        ...
        inner join payslip_period as PP
            on PP.id = HP.payslip_period_id
        where HPC.code = 'PRIM_LIQ'
            and HP.state = 'done'
            and HPC.contract_id = {contrato} and HPC.payslip_id != {nomina}
            and PP.start_period <= '{ep}' and PP.end_period >= '{sp}'
        """.format(contrato=ld['contract'].id, nomina=self.id, ep=ref_to_date, sp=ref_date)
        prev_prim_liq = orm.fetchall(self._cr, prev_prim_liq)
        if prev_prim_liq:
            prev_prim_liq = prev_prim_liq[0][0] if prev_prim_liq[0][0] else 0
            lr[0] = lr[0] - prev_prim_liq
            lr[6].append(('PRIMA LIQUIDADA EN OTRAS LIQUIDACIONES',prev_prim_liq))
    ...
    def _indem(self, ld):
        ...
        ref_to_date = ld['contract'].date_end or ld['period'].end_period
        ref_to_date = str(ref_to_date[0:8]) + str(ref_to_date[8:10] if int(ref_to_date[8:10]) < 31 else 30)
        if ld['contract'].separation_type in ('retnotjuscau', 'decunicom'):
            if term in ['fijo', 'obralabor']:
                ...
                inner join payslip_period as PP
                    on PP.id = HP.payslip_period_id
                where HPC.code = 'INDEM'
                    and HP.state = 'done'
                    and HPC.contract_id = {contrato} and HPC.payslip_id != {nomina}
                    and PP.start_period <= '{ep}' and PP.end_period >= '{sp}'
                """.format(contrato=ld['contract'].id, nomina=self.id, ep=ref_to_date, sp=ref_date)
                prev_indem = orm.fetchall(self._cr, prev_indem)
                if prev_indem:
                    prev_indem = prev_indem[0][0] if prev_indem[0][0] else 0
                    lr[0] = (lr[0]*lr[1] - prev_indem) / lr[1]
                    lr[6].append(('INDEMNIZACION EN OTRAS LIQUIDACIONES',prev_indem))
    ...
    def _fond_sol(self, ld):
        ...
        ibd = ld['concepts']['IBD']['total'] if 'IBD' in ld['concepts'] else 0
        if ibd > 4 * ld['sal_min_ev'] and self.env.user.company_id.cal_fond_sol_sub:
            retired = True if ld['contract'].fiscal_subtype_id.code not in ['00', False] else False
            if not retired:
    ...
    def _fond_sub(self, ld):
        ...
        if not retired:
            ...
            if 16 * sal_min <= ibd <= 17 * sal_min:
                ...
                rate += 1
        if rate and self.env.user.company_id.cal_fond_sol_sub:
            lr[0] = ibd
            lr[2] = rate
            if self.env.user.company_id.ded_round:
                amount = lr[0] * lr[1] * lr[2] / 100
    ...
    def _brtf(self, ld):
        ...
        nov_no_ret_sel = """
        select sum(HPN.total)
        from hr_payroll_novedades as HPN
        inner join hr_payroll_novedades_category as HPNC
            on HPNC.id = HPN.category_id and HPNC.ex_rent 
            and HPNC.concept_category in ('earnings','o_earnings','o_salarial_earnings','comp_earnings','o_rights')
        where HPN.contract_id = {contrato}
            and HPN.state = 'validated' 
            and HPN.approve_date between '{sd}' and '{ed}'""".format(
                contrato=ld['contract'].id, sd=ld['period'].start_period, ed=ld['period'].end_date)
        nov_no_ret = orm.fetchall(self._cr, nov_no_ret_sel)
        if nov_no_ret:
            if nov_no_ret[0][0]:
                taxed_inc -= nov_no_ret[0][0]
        
        trib_nov_no_ret_sel = """
        select sum(HPOT.valor)
        from hr_payroll_obligacion_tributaria as HPOT
        inner join hr_payroll_obligacion_tributaria_category as HPOTC
            on HPOTC.id = HPOT.category_id and HPOTC.ex_rent 
            and HPOTC.concept_category in ('earnings','o_earnings','o_salarial_earnings','comp_earnings','o_rights')
        where HPOT.contract_id = {contrato}
            and HPOT.state = 'validated'
            and HPOT.approve_date <= '{sd}' and '{ed}' <= HPOT.date_to""".format(
                contrato=ld['contract'].id, sd=ld['period'].start_period, ed=ld['period'].end_date)
        trib_nov_no_ret = orm.fetchall(self._cr, trib_nov_no_ret_sel)
        if trib_nov_no_ret:
            if trib_nov_no_ret[0][0]:
                taxed_inc -= trib_nov_no_ret[0][0]
        ...
        for c in ld['concepts']:
            ...
            if is_ded and is_reg and c != 'ANT':
        ...
        for p in ld['payslips_month']:
            for c in p.concept_ids:
                ...
                if is_ded and is_reg and c.code != 'RTEFTE' and c.code != 'ANT':
        ...
        if not discard:
            ...
            # Top40
            base40 = net_income * 0.4  if (net_income * 0.4) <= (uvt * 420) else uvt * 420
    ...
    def _rtefte(self, ld):
        ...
        if brtf:
            if ld['contract'].p2:
                ...
                b_uvt = proj_base / uvt
                rate, adj, step =  self.cumpute_table_rtf(b_uvt)
    ...
    def _rtf_indem(self, ld):
        ...
        if indem*30 > 204 * ld['uvt']:
            indem_25 = ld['concepts']['INDEM']['total'] * 0.25
            base = ld['concepts']['INDEM']['total'] * 0.75
            percentage = 20
            log = [
                ('Indemnizacion', indem),
                ('204 UVTs', 204 * ld['uvt']),
                ('Renta exenta', indem_25),
                ('Base', base),
                ('Porcentaje de base', percentage)]
            lr[6] = log
            lr[0] = base
            lr[1] = 1
            lr[2] = percentage
    ...
    def _prv_prim(self, ld):
        if self.env.user.company_id.simple_provisions:
            ...
            a_prima = ld['concepts']['PRIMA']['total'] if 'PRIMA' in ld['concepts'] else 0
            ...
            total_earn = (earnings + prev_earnings + o_salarial_earnings + aux_trans + prev_o_sal_earn + o_rights +
                          prev_o_rights + comp_earnings + prev_comp_earn + prev_aux_trans - prima_prev - prv_prev - a_prima)
            lr[0] = total_earn
            lr[2] = 8.33
        else:
            # Calculo de fechas de referencia para calculos
    ...
    def _prv_ices(self, ld):
        if self.env.user.company_id.simple_provisions:
            ...
            net_ices = (ces_data['pres'] - ces_data['partials']) * 0.12 / 360 * days
            i_fecha = net_ices

            lr[0] = ces_data['pres']
            lr[5] = net_ices - t_provs + prev_ices
    ...
    def _prv_vac(self, ld):
        if self.env.user.company_id.simple_provisions:
            ...
            total_earn = earnings + prev_earnings + o_salarial_earnings + prev_o_sal_earn + o_rights + prev_o_rights - prima_prev - prv_prev - a_prima
            lr[0] = total_earn
            lr[2] = 4.17
        else:
            k_dt_start = ld['contract'].date_start
            ...
            if self.env.user.company_id.quote_prv_vac_without_comp_earnings:
                comp_earnings = []
                a_comp_earnings = 0
            ...
            if self.env.user.company_id.prv_vac_cpt:
                ...
                inner join hr_contract as HC
                    on HC.employee_id = RP.employee_id
                where HC.id = {contrato} 
                    and AML.state = 'valid'
                    and AML.account_id in {accounts}
                    and AML.date <= '{date_to}'""".format(
                        contrato=ld['contract'].id, accounts=accounts, date_to=ld['period'].end_period)

                provs = orm.fetchall(self._cr, provs_q)

                t_provs = 0 if not provs else provs[0][0] if provs[0][0] else 0
            ...
            if prev_vac_liq:
                ...
                prev_vac_liq = sum([x[0] for x in prev_vac_liq])
                reg_prov = avr * qty / 30
                lr[0] = (reg_prov - prev_vac_liq)
                lr[6].append(('TOTAL PROVISIONADO A LA FECHA', reg_prov))
                lr[6].append(('DIAS DE VACACIONES PROVISIONADOS', 0))
            elif qty:
                payslip_vac = self.leave_days_ids.filtered(lambda x: x.state == 'validate' and x.holiday_status_id.vacaciones)
                qty_disf = sum([aus.days_assigned for aus in payslip_vac])
                payed_per = ld['concepts']['VAC_PAG']['qty'] if 'VAC_PAG' in ld['concepts'] else 0
                ...
                lr[6].append(('TOTAL PROVISIONADO A LA FECHA', reg_prov))
                lr[6].append(('DIAS DE VACACIONES PROVISIONADOS', qty))
            else:
                lr[3] = 'na'
        return lr
