...
def is_dom_fes(date_in,festivos):
    #festivos = self.env['hr.holiday.lines']
    vlook_festivo = festivos.search([
        ('holiday_date', '=', date_in)
    ])
    is_dom = True if date_in.weekday() == 6 else False

    return True if is_dom or vlook_festivo else False
...
class HrContract(models.Model):
    ...
    state = fields.Selection([('in_progress', 'Progreso'), ('done', 'Terminado')],
                             default="in_progress", string='Estado', track_visibility='onchange')
    ...
    pending_vac = fields.Float('Dias pendientes de vacaciones', track_visibility='onchange')
    vac_book = fields.One2many('hr.vacation.book', 'contract_id', string="Libro de vacaciones", track_visibility='onchange')
    ...
    def compute_rtf2(self):
        ...
        for k in self:
            ...
            if k.ded_dependents:
                ded_depend = dep_base if dep_base <= 384 * uvt else 384 * uvt
            else:
                ded_depend = 0
            ...
            log += [('18.TOP 25% [(12-16-17)*25% o 2880 UVT]', top25)]
            ...
            rate, adj, step =  self.env['hr.payslip'].cumpute_table_rtf(b_uvt)
    ...
    def get_pend_vac(self, context=None, date_calc=False, sus=0):
        ...
        vac_book_q = ("SELECT vb.enjoyed, vb.payed, vb.licences "
                      "FROM hr_vacation_book vb "
                      "INNER JOIN hr_contract hc ON hc.id = vb.contract_id "
                      "LEFT JOIN hr_payslip hp ON hp.id = vb.payslip_id "
                      "WHERE hc.id = {c} "
                      "AND (hp.liquid_date <= '{d}' OR vb.payslip_id is Null)".format(c=self.id, d=date_calc))
        data = avancys_orm.fetchall(self._cr, vac_book_q)
        lic = sus
        taken = 0
        for x in data:
    ...
    def get_pend_vac_upd(self, context=None, date_calc=False, sus=0):
        for k in self:
            if k.type_id.type_class == 'apr':
                k.pending_vac = 0
                continue
            vac = ("SELECT hh.id, hh.number_of_days, hh.payed_vac, hh.payslip_id from hr_holidays hh "
                   "LEFT JOIN hr_holidays_status hhs on hhs.id = hh.holiday_status_id "
                   "WHERE hh.contract_id = {k} "
                   "and hh.at_date > '{init_vac}'"
                   "and hhs.vacaciones = 't' "
                   "and hh.state = 'paid' ".format(k=k.id, init_vac=self.env.user.company_id.init_vac_date))
            vacs = avancys_orm.fetchall(self._cr, vac)
            upd_book = []
            for v in vacs:
                payslip = v[3] if v[3] is not None else False
                if not payslip:
                    continue
                upd_book.append({
            ...
            where HPC.code = 'VAC_LIQ' and HP.state = 'done'
                and HP.contract_id = {contrato}
                and PP.start_period >= '{init_vac}'
            limit 1""".format(contrato=k.id, init_vac=self.env.user.company_id.init_vac_date)
            for vac_liq_slip in avancys_orm.fetchall(self._cr, qty_days_vac_liq):
                upd_book.append({
            ...
            #Tiene en cuenta las suspenciones
            sus_sel = """
            ...
            where HHD.name >= '{init_vac}'
                and HHD.state = 'paid' and HHD.contract_id = {contrato}
            group by HHD.payslip_id
            """.format(init_vac=self.env.user.company_id.init_vac_date, contrato=k.id)
            for sus_paid in avancys_orm.fetchall(self._cr, sus_sel):
                upd_book.append({
            ...
            vac_book_old = """
            select id from hr_vacation_book where contract_id = {k} and payslip_id is not Null
            """.format(k=k.id)
            vac_book_old = [x[0] for x in avancys_orm.fetchall(self._cr, vac_book_old)]
            if vac_book_old:
                avancys_orm.fast_delete(self._cr, 'hr_vacation_book', ('id', vac_book_old))

            avancys_orm.direct_create(self._cr, self._uid, 'hr_vacation_book', upd_book)
...
class HrPayslip(models.Model):
    ...
    def reckon_obligaciones(self):
        ...
        if self.tipo_nomina.code in ('Nomina', 'Vacaciones', 'Liquidacion'):
            ...
            SELECT HPOT.id FROM (
                SELECT HPOT.id, 
                bool_and(CASE WHEN HPOTL.id IS NULL THEN TRUE WHEN HP.payslip_period_id != {period} THEN TRUE
                    WHEN HP.state = 'draft' THEN TRUE WHEN HPOTC.worked_days_depends THEN TRUE 
                    ELSE FALSE END) as "condition"
                FROM hr_payroll_obligacion_tributaria AS HPOT
                INNER JOIN hr_payroll_obligacion_tributaria_category as HPOTC on HPOTC.id = HPOT.category_id
                LEFT JOIN hr_payslip_obligacion_tributaria_line AS HPOTL on HPOT.id = HPOTL.obligacion_id
                LEFT JOIN hr_payslip AS HP on HP.id = HPOTL.payslip_id
                WHERE HPOT.state in ('validated','done') AND HPOT.contract_id = {contract}
                AND HPOT.date_from <= '{date_to}' AND (HPOT.date_to >= '{date_from}' OR HPOT.date_to is Null)
                AND (HPOT.bm_type = '{bmt}' OR HPOT.bm_type = 'both' OR HPOT.bm_type = '{dum}')
                GROUP BY HPOT.id) as HPOT
            WHERE HPOT.condition""".format(contract=self.contract_id.id,
                                  date_from=self.payslip_period_id.start_period, date_to=self.payslip_period_id.end_period,
                                  bmt=bmt, dum=dum, period=self.payslip_period_id.id))
        ...
        for slip in self:
            if slip.state == 'draft':
                ...
                if slip.move_name:
                    ...
                    name = self.env['ir.sequence'].next_by_id(journal_seq.id)
                account_period = self.env['account.period'].find(slip.liquid_date)[0]
                if account_period.state == 'done':
                    raise Warning("El periodo del movimiento contable a generar ya se encuentra cerrado")

                #INC-0000033078
                if account_period.state == 'done':
                    raise Warning("No se puede causar en un periodo cerrado")
                ...
                for c_line in slip.concept_ids:
                    if c_line.total >= 0:
                        ...
                        if c_line.code == 'NETO' and self.env.user.company_id.positive_net:
                            raise Warning(u"La nomina de {e} presenta neto negativo lo cual no es "
                                          u"permitido segun la politica configurada en la compaía".format(e=slip.employee_id.name))
                    ...
                    if debit_account not in [None, False] and amount:
                        ...
                        if analytic_id or customer_id:
                            line_data['analytic_id'] = analytic_id
                            line_data['customer_id'] = customer_id
                        if c_line.concept_id.id:
                            if c_line.concept_id.is_retention:
                                line_data['base_amount'] = c_line.amount
                                line_data['tax_amount'] = c_line.total
                                line_data['base_code_id'] = c_line.concept_id.code_tax.id
                        move_line_data.append(line_data)
                ...
                if move_line_ids:
                    ...
                    if sums:
                        debcreddif = sums[0][0] - sums[0][1]
                if debcreddif and abs(debcreddif) > slip.company_id.limit_ajuste_peso:
                    raise Warning(u"El valor de diferencia encontrado en la causacion de la nomina de {e} es {d} "
                                  u"y es superior al configurado como valor maximo de ajuste al peso. \n "
                                  u"Para configurarlo dirijase a Ajustes/Cartera y para corregir el error configure "
                                  u"las cuentas contables faltantes en los conceptos de nomina "
                                  u"calculados".format(e=slip.employee_id.name, d=debcreddif))

                if debcreddif:
                    self._cr.execute("select id from account_config_settings_cartera limit 1")
                    res = self._cr.fetchone()
                    if res and res[0]:
                        adj_id = res[0]
                        adjustment_obj = self.env['account.config.settings.cartera'].browse(adj_id)
                    else:
                        adjustment_obj = self.env['account.config.settings.cartera']

                    if debcreddif > 0:
                        account_id = adjustment_obj.account_ajuste_id or slip.journal_id.default_credit_account_id
                        direction = 'credit'
                    else:
                        account_id = adjustment_obj.account_ajuste_id2 or slip.journal_id.default_debit_account_id
                        direction = 'debit'
                    analytic_id, customer_id = False, False
                    analytic_account_id = slip.contract_id.analytic_account_id
                    if str(account_id.code[0]) not in ['4', '5', '6', '7']:
                    ...
                    for line_id in move_line_ids:
                        ...
                        if line.analytic_account_id:
                            analytic_line = {
                                ...
                                'ref': slip.number,
                                'move_id': line_id[0],
                                # 'partner_id': line_id.partner_id.id,  # testing, eliminar si inc
                                'journal_id': slip.journal_id.analytic_journal_id.id,
                                'general_account_id': line.account_id.id,
                                'amount': line.credit - line.debit,
                            }
                ...
                # Ausencias: Se pasan a pagadas unicamente si termina dentro del periodo de la nomina
                # TODO: Arreglar logica de escritura en libro de vacaciones que no es clara.
                for holiday in slip.leave_ids:
                    if not holiday.payslip_id:
                            ...
                            for days_line in holiday.line_ids:
                                if not days_line.payslip_id:
                                    avancys_orm.direct_update(self._cr, 'hr_holidays_days',
                                                              {'payslip_id': slip.id, 'state': 'paid'},
                                                              ('id', days_line.id))
                                    if slip.payslip_period_id.end_period >= days_line.name:    
                                        day_vac = datetime.strptime(days_line.name, "%Y-%m-%d")
                                        domfes = is_dom_fes(day_vac, festivos)
                                        if domfes:
                                            continue
                                        enjoyed += 1
                        ...
                        elif holiday.date_to[0:10] <= slip.payslip_period_id.end_period and not holiday.holiday_status_id.vacaciones:
                            avancys_orm.direct_update(self._cr, 'hr_holidays', {'payslip_id': slip.id, 'state': 'paid'},
                                                      ('id', holiday.id))
                            for day in holiday.line_ids:
                                if not day.payslip_id:
                                    avancys_orm.direct_update(self._cr, 'hr_holidays_days',
                                                              {'payslip_id': slip.id, 'state': 'paid'},
                                                              ('id', day.id))
                        ...
                        if holiday.holiday_status_id.vacaciones and self.env.user.company_id.fragment_vac:
                            enjoyed = 0
                            if holiday.date_to[0:10] <= slip.payslip_period_id.end_period:
                                avancys_orm.direct_update(self._cr, 'hr_holidays', {'payslip_id': slip.id, 'state': 'paid'},
                                                      ('id', holiday.id))
                            for leave in slip.leave_days_ids:
                                if not leave.payslip_id:
                ...
                for holiday_day in slip.leave_days_ids:
                    avancys_orm.direct_update(self._cr, 'hr_holidays_days', {'payslip_id': slip.id, 'state': 'paid'}, ('id', holiday_day.id))
                ...
                if slip.tipo_nomina.code == 'Liquidacion':
                    ...
                    slip.contract_id.get_pend_vac_upd()
            i += 1
            bar = avancys_orm.progress_bar(i, j, bar, slip.id)
        ...
        for slip in self:
            ...
            if slip.state == 'done':
                ...
                if slip.move_id:
                    ...
                    self._cr.execute("UPDATE hr_payslip set move_id = Null where id = {p}".format(p=slip.id))
                    analytic_lines_query = """  select aal.id
                                                from account_analytic_line as aal
                                                inner join account_move_line as aml
                                                    on aml.id = aal.move_id
                                                inner join account_move as am
                                                    on am.id = aml.move_id
                                                where am.id = {asiento}""".format(asiento=move_id)
                    analytic_lines_ids = avancys_orm.fetchall(self._cr, analytic_lines_query)
                    analytic_lines_ids = [aal[0] for aal in analytic_lines_ids]
                    if analytic_lines_ids:
                        avancys_orm.fast_delete(self._cr, 'account_analytic_line', ('id', analytic_lines_ids))
                    avancys_orm.fast_delete(self._cr, 'account_move_line', ('move_id', move_id))
                    avancys_orm.fast_delete(self._cr, 'account_move', ('id', move_id))
                # Prestamos
