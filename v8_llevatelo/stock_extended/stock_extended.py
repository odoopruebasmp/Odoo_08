...
class stock_move(osv.osv):
    ...
    _columns = dict(
        ...
        product_uos=fields.many2one('product.uom', 'Unit of Measure',  readonly=True),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not sale:
            ...
            if vals.get('purchase_line_id'):
                ...
                costo_adicional = (0.0 if vals.get('split_from') or vals.get('propagate')
                    else purchase_line_obj.mayor_valor(cr, uid, purchase_line_id, context=context))
                ...
                vals.update({'product_uos': purchase_line_id.product_id.uom_id.id})
