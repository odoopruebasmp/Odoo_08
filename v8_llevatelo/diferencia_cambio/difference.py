        ...
        if move_ids:
            for move in move_ids:
                ...
                if amount > 0.0 and move.account_id.type == 'receivable':
                    ...
                    raise Warning('Error en la cuenta %s - %s, Verifique que si existe la configuracion para Diferencia en cambio'
                                  ' el tipo interno sea A cobrar o A pagar, de lo contrario retire la configuracion '
                                  'para Diferencia en cambio.'.encode('utf-8') % (move.account_id.code, move.account_id.name))
