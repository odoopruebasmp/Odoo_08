...
class mrp_production(osv.osv):
    ...
    def action_produce(self, cr, uid, production_id, production_qty, production_mode, wiz=False, context=None):
        ...
        if production_mode == 'consume_produce':
            ...
            costo_anterior = 0
            for move in production.move_created_ids2.filtered(lambda m: m.state != 'cancel'):
...
class stock_move_consume(osv.osv_memory):
    ...
    def get_qty_requested_per_product_per_location(self, production):
        ...
        return sorted([
            (line.product_id.id,
            sum(
                i.product_qty for i in self
                if i.product_id.id == line.product_id.id
            ),
            location.id)
            for line in self if line.product_id.type == 'product'
    ...
    def _get_lot_unavailable(self, line, production):
        ...
        """.format(
            move_pool, line.restrict_lot_id.id or -1, line.product_id.id,
                location.id
        ))
        qty_available = self._cr.fetchone()
    ...
    def validate_quantities_to_transfer(self, production):
        ...
        item_lot_not_available = filter(
            lambda line: (self._get_lot_unavailable(line, production)
                ...
                and line.restrict_lot_id),
            self
        )
        if item_lot_not_available:
            separator = ','
            lots = separator.join([
                item.restrict_lot_id.name for item in item_lot_not_available
            ])
            products = separator.join([
    ...
    def change_prod_qty(self, cr, uid, ids, context=None):
        ...
        for wiz_qty in self.browse(cr, uid, ids, context=context):
            ...
            for move in prod.move_lines:
                ...
                consumed_product_qty = sum(
                    m.product_qty for m in prod.move_lines2\
                    if m.product_id.id == move.product_id.id\
                    and m.product_uom.id == move.product_uom.id)
                for r in product_details:
                    if r['product_id'] == move.product_id.id:
                        move_obj.write(cr, uid, [move.id], {'product_uom_qty': max(r['product_qty'] - consumed_product_qty, 0)})
